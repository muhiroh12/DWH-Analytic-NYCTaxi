...
pickup_times AS (
    ...
    FROM 
        {{ ref('fct_trips') }} fct
    JOIN {{ ref('dim_time') }} pu_t ON fct.pickup_time_id = pu_t.time_id
),
dropoff_times AS (
    SELECT 
        fct.trip_id,
    ...
    FROM 
        {{ ref('fct_trips') }} fct
    JOIN {{ ref('dim_time') }} pu_t ON fct.pickup_time_id = pu_t.time_id
    JOIN {{ ref('dim_time') }} do_t ON fct.dropoff_time_id = do_t.time_id
),
trip_details AS (
    SELECT 
        fct.trip_id,
    ...
    FROM 
        {{ ref('fct_trips') }} fct
    JOIN {{ ref('dim_vendor') }} v ON fct.vendor_id = v.id
    JOIN {{ ref('dim_payment') }} p ON fct.payment_type = p.type_id
    JOIN {{ ref('dim_zone') }} z_pickup ON fct.pu_location_id = z_pickup.location_id
    JOIN {{ ref('dim_zone') }} z_dropoff ON fct.do_location_id = z_dropoff.location_id
    JOIN {{ ref('dim_ratecode') }} r ON fct.rate_code_id = r.rate_id
    JOIN {{ ref('dim_triptype') }} tt ON fct.trip_type = tt.type_id
    WHERE 
